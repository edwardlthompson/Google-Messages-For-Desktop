...
Unsigned `.deb` install on GNOME or KDE Wayland:
...
2. Tray: needs a working StatusNotifier/AppIndicator. If the tray is missing, use Settings → Tray icon → Enable Tray Icon after installing `gnome-shell-extension-appindicator` (or KDE’s tray).
...
```bash
cd electron && npm ci && npm run package:linux
...
`package:linux` builds only the `.deb` (and verifies no leftover AppImage/zip in `electron/dist/`). The smoke installs that `.deb`, checks `.desktop` `StartupWMClass` + `SingleMainWindow` + sms/tel/im MimeTypes, launches once, confirms a second launch exits, then quits. It does **not** cover Google sign-in or the Wayland checklist above. Pass `--uninstall` to remove the package after smoke.
