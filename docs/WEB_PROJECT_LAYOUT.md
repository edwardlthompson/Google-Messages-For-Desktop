...
2. `.github/workflows/pages.yml` (template; this child defers Pages) runs `npm ci`, `npm run build` with `VITE_BASE_PATH=/${{ github.event.repository.name }}/`.
...
1. Delete or disable `.github/workflows/pages.yml` (already deferred on this child).
