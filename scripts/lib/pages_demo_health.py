...
def check_readme(root: Path | None = None) -> list[str]:
    base = root or ROOT
    if not (base / ".github" / "workflows" / "pages.yml").is_file():
        return []
...
def main() -> int:
    ...
    if not (Path.cwd() / ".github" / "workflows" / "pages.yml").is_file():
        print("Pages demo link health skipped (no pages.yml)")
        return 0
    if errors:
        ...
        return 1
    if url is None:
        print("Pages demo link health skipped (no github_repo url)")
        return 0
    status = probe(url)
