...
class CiGapsTests(unittest.TestCase):
    ...
    def test_accepts_master_without_ci_ok(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            schema = root / "schemas"
            schema.mkdir()
            gaps = [{"id": gid, "status": "accepted", "note": "n"} for gid in REQUIRED_IDS]
            schema.joinpath("ci-gaps.json").write_text(
                json.dumps({"issue_label": "ci-gap", "gaps": gaps}), encoding="utf-8"
            )
            dest = root / ".github" / "ISSUE_TEMPLATE"
            dest.mkdir(parents=True)
            dest.joinpath("template_improvement.yml").write_text(
